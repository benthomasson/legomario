...
class BTLegoMario(BTLego):
        ...
        def which_device(advertisement_data):
                ...
                if 919 in advertisement_data.manufacturer_data:
                        ...
                        # 00 Option (unused)
                        if advertisement_data.manufacturer_data[919][1] == 0x45:
                                return 'peach'
                                ...
                                print(str(advertisement_data.manufacturer_data[919][1]))
...
#                                elif subscription == 'error'
        ...
        def decode_accel_data(self, data):
                ...
                # 0x8 0x0 0x45 0x0 [ 0x0 0x80 0x0 0x80 ]
                        ...
                        HARDSHAKE = 0x80008  # ???
                        SPIN =  0x200020 # ???
                        INVERSE_MASK = ~(BUMP | SHAKE | TURNING | FASTMOVE | TRANSLATION | HIGHFALLCRASH | REVERSE | JUMP| DIRECTIONCHANGE) & 0xffffffff
                        data_int = int.from_bytes(data[:-1], byteorder="big", signed=False)
                        if data_int:
                            BTLegoMario.dp(self.which_brother+" gesture data: " + hex(data_int))
                        if data_int & BUMP:
                            print('bump')
                            pass
                        if data_int & SHAKE:
                        ...
                        if data_int & TURNING:
                            print('turning')
                            pass
                        if data_int & FASTMOVE:
                            #print('fastmove')
                            pass
                        if data_int & TRANSLATION:
                            #print('translation')
                            pass
                        if data_int & HIGHFALLCRASH:
                            #print('highfallcrash')
                            pass
                        if data_int & DIRECTIONCHANGE:
                        ...
                        if data_int & FLIP:
                            print('flip?')
                        if data_int & HARDSHAKE:
                            print('hardshake?')
                        if data_int & ROLL:
                            print('roll?')
                        if data_int & SPIN:
                            print('spin?')
                        if data_int & INVERSE_MASK:
                            print("??? " + hex(data_int & INVERSE_MASK))
